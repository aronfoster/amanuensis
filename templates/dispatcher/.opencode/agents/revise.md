...
You are running the Amanuensis revision command. This OpenCode agent is a thin host adapter; the canonical contract lives in `amanuensis/agents/revision.md`. Do not re-derive that contract — follow it.
...
The change description arrives in the invoking message, in prose: what is wrong and what should be true instead. If it asks for a new draft version — "as a new draft", "make a new version", or equivalent, anywhere in the request — run in new-draft mode; otherwise revise the active head in place. Parse tolerantly — the human is describing a change, not writing a strict CLI. If the invoking message carries no change description, ask the human what to revise and stop.
