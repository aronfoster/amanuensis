...
The requested step_id arrives in the invoking message, optionally followed by a read-from draft: `<step_id> [from <draft-vNN>]`. Parse it tolerantly — the human is writing prose, not a strict CLI: accept `from draft-vNN` or a bare `draft-vNN` after the step_id. The read-from draft overrides which draft `<latest-draft>` resolves to for this one invocation and nothing else; it is how a human branches a rerun from an earlier draft. If the invoking message names no step_id, ask the human which step to run and stop.
