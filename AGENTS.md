...
- `templates/dispatcher/.claude/commands/revise.md` — Claude Code slash command implementing the targeted-revision command per `agents/revision.md`.
...
- `agents/steps/storyboarding.md` — produces per-beat storyboard blocks from the chapter's scene list, summary, character knowledge, and canon, populating `reader_takeaway` for every block and deriving `Reader state in` / `Since previous scene` once per scene so isolated drafters have explicit entry context.
- `agents/steps/storyboard-review.md` — advisory, report-only pass over the chapter's storyboard blocks that checks each beat's `reader_takeaway`, reveal setup/timing against `reveals.md`, and the placement, correctness, and scope of each scene's entry context, producing `storyboards/storyboard-review.md`.
- `agents/steps/drafting.md` — chapter coordinator that validates scene-entry context, dispatches isolated per-scene subagents with only the scene's storyboards and selected voice file, and assembles their output into a single draft; entry context is normally kept implicit rather than recapped.
- `agents/steps/compliance-report.md` — the bounded **relational** review: checks the draft against storyboard Must-Contain / Must-Not-Contain requirements and canon (local checks), and against maintained state, targeted source context, and `Reader state in` / `Since previous scene` (relational checks), including purposeless re-establishment of already-known context, producing an annotated `reviewer-actions.md` report with cited referents, defect-type labels, and a `## Context consulted` record.
...
- `agents/review-context.md` — the review-context strategy: review classification and minimum-context-per-class (including explicit prior-reader scene-entry context), referent identification and targeted retrieval, the precedence orders, the defect taxonomy that routes remediation, the project-scale/type gradation, and the audit table. The single source the relational reviews (`compliance_report`, `storyboard_review`) reference for *what context a review needs*.
- `agents/reveals.md` — the story-level reveals-ledger schema: the human-authored, `role: planning`, project-root `reveals.md` indexing forward reveals by `id` with `lands:` / `setup:` / `concealed-until:` block-qualified positions, consumed read-only by `storyboard_review`. Distinct from the book-level `continuity.md` risk notes.
