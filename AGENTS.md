...
- `agents/steps/storyboard-review.md` — advisory, report-only pass over the chapter's storyboard blocks that checks each beat's `reader_takeaway` is supported and — now reasoning **cross-chapter** against the `reveals.md` ledger — that each reveal's setup is sufficient and no block discloses a ledger secret before its `concealed-until:`, producing `storyboards/storyboard-review.md`.
...
- `agents/review-context.md` — the review-context strategy: review classification and minimum-context-per-class, referent identification and targeted retrieval, the precedence orders (review tiebreak, reveal-timing carve-out, authoring authority), the defect taxonomy that routes remediation, the project-scale/type gradation, and the audit table. The single source the retrofitted relational reviews (`compliance_report`, `storyboard_review`) reference for *what context a review needs*.
