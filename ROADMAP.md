...
Notes: Sprint 16 plans M11 (see SPRINT.md). Locked there (owner decisions): the bare-`FIX` fallback on categories with no bare-FIX rule stays fixer-level treat-as-`ESCALATE`, recorded in the apply log — the validator stays category-agnostic with family-level token rules (rejected: per-category payload keys in the grammar YAML and validator); the companion may write a legal `BULK:` header when the human states a bulk decision — its write surface expands to decision fields plus legal bulk headers (rejected: bulk headers stay hand-edit-only); the progress ledger is unchanged — the `inherited-by-bulk` bucket is the item-unit/category-unit distinction (rejected: an explicit bulk-adjudicated-categories ledger line). Also locked at planning: the shipped validator's bulk logic already produces the anti-AI fixture's documented ledger and rejects an illegal bulk header (verified against a scratch grammar copy with the marker flipped), so M11.3 is the marker flip plus **container settlement** — `anti_ai` gets `container_pattern` keys and the validator a generic `container_exempt_prefix` (the `### Summary — Scene <id>` heading varies by scene id, so the suffix mechanism cannot exempt it), because without a container check a pre-M11 positional report validates as proceed/exit 0, the silent-pass the compliance container check exists to prevent.
