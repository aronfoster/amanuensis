...
- **Heading-item families need an orphan-anchor check (surfaced in M12 PR review).** For families whose `item_line_pattern` is a heading (`metaphor`'s `^### `, and `prose_pass`'s `^##### ` migrated in M12), the validator's `container_pattern` check catches a *fully* unanchored section (zero anchored units → invalid) and an orphaned `- Decision:` field, but it does **not** flag an item-line heading that carries neither an anchor nor a decision field — so a partially hand-migrated report with a leftover legacy finding (an unanchored `##### ` heading with an old `Annotation:` line and no `Decision:`) can slip through uncounted. The container check's guarantee is therefore "the section is not entirely anchorless," not "every heading-item is a unit." M13 already owns heading-item container/anchor semantics (the note above), so fold the fix in here: add a validator rule that an `item_line_pattern` heading with no immediately-preceding anchor is an orphaned item, scoped to heading-item families so it does not misfire on the bullet-item families (`compliance`/`anti_ai` use `item_line_pattern: "^- "`, which also matches `- Decision:` and auxiliary lines). Until then, M12/M13 docs describe the container check as the zero-anchored-units guarantee it is, not the stronger every-heading guarantee.
