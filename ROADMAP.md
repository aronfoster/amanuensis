...
- [ ] M12.1 Retarget `prose_pass` findings format: anchor plus blank `Decision:` / `Decision-note:` fields on **every** finding (`KEEP` included); the zero-findings marker (`#### Findings — none` heading plus a `No findings.` line) per grammar; the annotation-grammar section (with its bracketed placeholder) moves to a `review-grammars.yaml` reference. The reviewer's `KEEP/TIGHTEN/FLATTEN/REWRITE` rubric is unchanged — only the finding template gains the anchor and fields.
- [ ] M12.2 Update `prose_fix` to consume `Decision:` fields via the shared validator (per-unit pending gate; positional annotations are invalid input); delete the effective-annotation resolution, including the `KEEP`→`SKIP` shortcut — every finding now carries an explicit decision; add the `KEEP` severity case (a bare `FIX` on a `KEEP` finding has no fix strategy, so it falls back to `ESCALATE`; `FIX: <instruction>` applies the instruction sentence-locally).
- [ ] M12.3 Flip prose-pass to `adopted` in `review-grammars.yaml` with the container shape settled (`container_pattern: "#### Findings"`, `container_exempt_suffix: "— none"`); no `fanout_categories` declared; `item_shape` / `counting` / `blank_means` / `review_id_item` rewritten so every finding is a unit; restate the family's `bulk_rules` in the post-Sprint-16 vocabulary (artifact bulk is retired everywhere; per-entry is this family's point).
- [ ] M12.4 Companion support for prose-pass's flat (non-category) finding queue: the skill's generic adopted-family path already covers it (compliance is the precedent); no category queues and no fan-out offers anywhere in this family.
- [ ] M12.5 Smoke coverage: non-`KEEP` finding pending/decided flow; an all-`KEEP` report (every finding anchored, decided `SKIP`) validating clean; an unanchored pre-M12 report rejected as invalid.
