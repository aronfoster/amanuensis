...
- M14 knowledge-freshness per source chapter (surfaced in Sprint 20 / M15 PR review) — the
  knowledge-freshness predicate as written (`agents/characters.md:89`,
  `agents/steps/scene-knowledge-update.md:71`) says "the attempt manifest" without disambiguating
  which chapter's. For `book`/`series`, `knowledge/book-N.md` aggregates entries across chapters
  whose per-chapter `draft-vNN.md` basenames repeat, so an entry's `committed-in` is meaningful only
  against the manifest of the chapter named in its `story-position`; comparing against the invoked
  chapter's lineage can falsely mark a prior-chapter entry stale or falsely fresh on a basename
  collision. Masked today because M14's demo is a single-chapter `short_story`. M15 states the
  resolved rule for `continuity/`; apply the same one-line wording fix to the M14 knowledge predicate.
