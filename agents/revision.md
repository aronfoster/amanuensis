...
**Never edited:** `pipeline-state.md`; side-artifact `Reviewed-draft:` stamps; manifest entries for existing drafts; the `## Superseded` / `## Lost or superseded` transition sections of the non-destructive temporal-state files (`continuity/`, character `knowledge/`); and archived material — any attempt directory other than `<latest-attempt>`, any `draft-vNN.md` other than the draft being revised, and any directory the consuming project keeps as an archive (e.g. a root `archive/`). Superseded and archived files record what was true when they were written. A revision changes the present, never the record.

**Current state vs. the record (non-destructive temporal-state files).** The temporal-state files that record change non-destructively — `continuity/` (objective facts) and M14's character `knowledge/`, `timeline.md`, and `relationships.md` — split into two parts a revision treats differently. Their **current-state entries carry current truth**, so an authorial revision corrects them **in place**, keeping each entry's `id`. Their **`## Superseded` / `## Lost or superseded` transition sections are the record** of what was true at an earlier story position, and a revision leaves them untouched. This is the diegetic-vs-authorial distinction stated once in `agents/continuity.md`: a *diegetic* change (the story advances and the fact changes in the fiction) is the writer step's non-destructive transition; an *authorial* change (a human `revise` fixing what the fiction says) was never true-in-the-fiction — the earlier value was a draft being corrected, whose record is the archived drafts — so the current entry is corrected in place and **no transition is added**, exactly as a revision edits the active-head draft in place while leaving superseded drafts as the record. A transition added by a revision would fabricate an in-story change that never happened.
