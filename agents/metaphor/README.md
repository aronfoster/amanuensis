...
5. **`metaphor_apply`** runs against the post-selection `metaphors.md`, applies the surviving variants to `<latest-draft>`, and writes the next `draft-vNN.md` (appending a per-version entry to `draft-manifest.md`). Workshop entries arrive as bare sentences (since workshop's integration phase was removed); the apply step's existing sentence-variant branch handles them.
