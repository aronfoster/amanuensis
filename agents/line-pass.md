...
- `xx-yy-draft-metaphor.md` — the prose after metaphor-apply has run; the source for unedited chunks and following-context windows
- `xx-yy-draft-line.md` — the in-progress output; the source for preceding-context windows once any chunk has been written
- The selected voice file or profile — passed in full as the system message; calibration anchor for the whole pass
...
`xx-yy-draft-line.md` in the chapter folder. Identical to `xx-yy-draft-metaphor.md` except for the sentence-level edits this pass produces.

Do not modify `xx-yy-draft-metaphor.md`.

At the end of `xx-yy-draft-line.md`, append a block comment apply log (see below).
...
- **Preceding context** comes from `xx-yy-draft-line.md` if the prior region has already been processed; otherwise from `xx-yy-draft-metaphor.md`. The editor calibrates against the chapter's actual evolving rhythm rather than against a frozen pre-edit state.
- **Following context** always comes from `xx-yy-draft-metaphor.md`. The downstream prose has not been edited yet.
...
Once a chunk has been written to `xx-yy-draft-line.md`, it is finalized. Subsequent chunks do not modify it. Each chunk's editor sees previous output as read-only preceding context and adjusts the current center to match — never the other way around.
...
**Step 4: Append to `xx-yy-draft-line.md`.** Write the rewritten center to the line-draft file, preserving its position in the chapter.
...
At the end of `xx-yy-draft-line.md`, append:
...
**Retroactive seam repair.** Once a chunk is finalized in `xx-yy-draft-line.md`, subsequent chunks do not modify it. Each editor adjusts the current center to match the prior output, never the reverse.
