...
Before dispatch, locate the first block by `beat_index` and confirm it contains `Reader state in` and `Since previous scene`. Match normalized heading text regardless of Markdown heading depth; `##` is canonical for new output, and `#` remains readable for legacy input. These sections are authoritative planning context for how this isolated scene follows what the reader has already experienced. They are not prose requirements and do not join `must_preserve`.
