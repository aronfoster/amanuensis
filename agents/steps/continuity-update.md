...
4. **Reconcile the confirmed load-bearing facts into the project-type target.** The target is `continuity/book-N.md` for `book` and `series` projects and `continuity/story.md` for `short_story` projects (create it if absent). Before writing, read the target's frontmatter `edit_policy` (Rule 7, `agents/update-rules.md:62-108`): a `locked` or `propose_only` target is **not** written — record an open question describing the fact, the target, and the routing decision, exactly as capture does for a blocked target. For a writable target:
   ...
   **Additive facts are not superseded.** Not every advancing fact is a change to a single mutable value. A **chronology / time anchor** is additive — "Day 1 at scene01" and "Day 12 at scene10" are *both* permanently true, each at its own position — so an advancing anchor is recorded as a **new** entry (above), never a `## Superseded` transition over the earlier one; **event staging** is the same (each named event's staging is its own fact). A later scene naming an *earlier* time than a still-current anchor is not an advance; if it **asserts the current timeline has regressed** it is a **conflict** (below), but an **intentional flashback, recollection, or recounted scene** is neither an advance nor a conflict — narrative order and story-world time are not always monotonic, so record its anchor at the story-world time it depicts. Only single-valued mutable state — location, possession, physical condition, role / assignment, and an **open causal thread's status** (open → resolved when later prose answers it, superseding the open entry while keeping its `id` so a character `truth:` pointer chases forward) — is superseded when it changes (`agents/continuity.md`).
   - A **conflict** — the prose contradicts a **still-current** maintained fact in a way that is **not** an intended change (a day count that **unintentionally** regresses — not a flashback; a role or possession that collides with an unretired entry; a fact that contradicts settled canon) — is **surfaced to `open-questions.md`** naming **both sides**: the prior entry's `id` + value + `evidence`, and the prose position + a quote. The prior entry is left **in place**, and the conflicting value is **not** written as a fact. Conflict-surfacing is **non-blocking** (like capture): the step continues with the other facts and completes; the human resolves the conflict out-of-band. This is the concrete meaning of "surfaces conflicts rather than silently choosing a version." (An *intended* diegetic advance is not a conflict — it is the transition above.)
...
Re-running this step against the **current active head** appends only genuinely new facts, corrections, and newly-detected conflicts, and duplicates nothing. Before appending, match each confirmed fact against the file's existing entries by `id` and by `(story-position + fact)`: a fact already recorded against the current active head is **not** re-appended; a fact that has changed **diegetically** — the new value appears at a **later** story position — is recorded once as a `## Superseded` transition plus a new stamped entry, not re-appended on every run; and a still-standing conflict is re-surfaced but not duplicated.

**Draft-level corrections are refreshed in place, not superseded.** On every rerun the step re-confirms each fact against the **current** active prose (step 2). When that confirmation yields a **different value at the *same* story position** as an existing entry — because a **fix step rewrote that scene**, whether by a **linear** advance to a corrected draft or a **branch** from an earlier one — this is an **authorial / draft-level** change, not a diegetic one: the story did not advance; the earlier value was a draft being corrected, whose record is the archived draft. Refresh that entry **in place** — its value, `committed-in`, and `evidence:` move to the active head, keeping its `id` — rather than appending a `## Superseded` transition (which would fabricate an in-story change that never happened and make point-in-time reconstruction false) or surfacing a conflict (the draft was legitimately corrected, not contradicted). The operative trigger is the **same-position value mismatch against the active head**, **not** the `derived-stale` / `derived-unsupported` bits: a *linear* correction leaves the predecessor draft inside the active head's lineage and its `evidence:` still resolving — so neither bit fires — yet the entry must still be refreshed. This is the diegetic-vs-authorial distinction (`agents/continuity.md`) applied to the rerun: **same** story position → in-place refresh; **later** story position → transition.

A rerun therefore **converges** — it never accumulates duplicate entries, duplicate transitions, or duplicate conflicts for the same committed fact. **Rebuild is this rerun-reconcile**: re-running against the current active head reconciles what the accepted prose committed — appending genuine diegetic changes as transitions, refreshing draft-level corrections in place, and re-surfacing conflicts — never fabricating an in-story change that did not happen.
...
**Overwriting or deleting a *diegetically*-superseded entry.** A **diegetic** change — a new value at a *later* story position — is always a `## Superseded` transition plus a new stamped entry, never an in-place rewrite; overwriting a diegetically-superseded entry destroys point-in-time reconstruction and breaks the non-destruction invariant. (A **draft-level correction** at the *same* story position is the deliberate exception: it is refreshed in place, because the earlier value was never true-in-the-fiction and the archived draft is its record — see Idempotency/rerun. The two are distinguished by story position, never conflated.)
