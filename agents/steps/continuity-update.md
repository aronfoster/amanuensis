...
**Draft-level corrections are refreshed in place, not superseded.** When the rerun runs against a **new active head produced by a fix-step rerun or branch** (the lineage/supersession model, `agents/project-layouts.md`) and the corrected prose commits a **different value at the *same* story position** as an existing entry — so that entry is now **derived-stale** or **derived-unsupported**, its `committed-in`/`evidence:` naming a draft the branch displaced — this is an **authorial / draft-level** change, not a diegetic one: the story did not advance; the earlier value was a draft being corrected, whose record is the archived draft. Refresh that entry **in place** — its value, `committed-in`, and `evidence:` move to the active head, keeping its `id` — rather than appending a `## Superseded` transition (which would fabricate an in-story change that never happened and make point-in-time reconstruction false) or surfacing a conflict (the draft was legitimately corrected, not contradicted). This is the diegetic-vs-authorial distinction (`agents/continuity.md`) applied to the rerun: **same** story position → in-place refresh; **later** story position → transition.
