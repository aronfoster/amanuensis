...
- `continuity/` (`continuity/book-N.md` for `book`/`series`, `continuity/story.md` for `short_story`) — the maintained derived-continuity state, read for the relational continuity checks (chronology, event-staging, location, possession, physical-condition, role, open-thread — including recalled-event-staging and recap fidelity). `required: false` and project-type-aware: `continuity/` exists meaningfully only for `book`/`series`, so a `short_story` must not block on its absence.
- `characters/<character-id>/knowledge/*.md` (`knowledge/book-N.md` for `book`/`series`, `knowledge/story.md` for `short_story`) — the maintained character-knowledge state, read for the `concealment_from_characters` / reveal-timing relational check. `required: false`, same project-type-aware rationale.
...
- **Canon consistency.** Where the prose asserts a checkable world-fact, confirm it against `canon_active` first, escalating to the *named* `canon/**` file only when `canon_active` is insufficient or does not cover it. A prose-vs-settled-canon contradiction is labeled `canon` (or `prose` where the prose is simply wrong against valid canon) by precedence.
