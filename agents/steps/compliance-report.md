...
Report compliance violations between a chapter's storyboard blocks and its drafted prose. This step is the **bounded relational review** (`agents/review-context.md`, `compliance_report` row of its audit): it is read-only, and it produces a per-block report of Must-Contain, Must-Not-Contain, and Canon-consistency violations (the local checks) **plus relational violations** — continuity, character-knowledge, reveal-timing, chronology, recollection, recap fidelity, and redundant re-establishment of prior-reader context, the drafted prose read against the maintained `continuity/` + `characters/<id>/knowledge/` state and named canon via targeted retrieval — for a human to triage. Fixing happens in the separate `compliance_fix` step. The report is the human review artifact that gates the fix step.

## Inputs

- `<chapter-folder>/storyboards/*-storyboard.md` — all storyboard blocks for the chapter. The block fields drive the three local checks and the prior-reader relational sub-check: `must_preserve` and `character_state_out` for Must-Contain; `concealment_from_reader` and `concealment_from_characters` for Must-Not-Contain; `canon_active` for Canon; and the first block's `Reader state in` / `Since previous scene`, read with the current `Beat`, `Must Preserve`, `Reader takeaway`, and `Craft signal`, for redundant re-establishment.
...
Read all storyboard blocks for the chapter in canonical scene and `beat_index` order. For each block, run the three **local** checks (1–3) against the corresponding prose range, and run the **relational** check (4) over the chapter as a whole against the maintained state + targeted retrieval. Record one entry per block in `reviewer-actions.md`.
...
- INCONSISTENT (chronology): [subject] — "[quote]" contradicts the maintained fact [what the referent fixes]. [defect: <type>] [ref: <referent>]
  - Decision:
  - Decision-note:
<!-- review-id: compliance:[scene-id]:block-NNN-v08 -->
- REDUNDANT (reader_state_in): [Item label] — "[quote]" re-establishes [already-known context] as new although the current storyboard assigns no new dramatic purpose. [defect: <type>] [ref: <storyboard-file>#reader-state-in]
  - Decision:
  - Decision-note:
```
...
A **relational** finding (Check 4) uses the same review-unit shape but its violation line carries the greppable trailing tag ` [defect: <type>] [ref: <referent>]` — `<type>` ∈ {prose, storyboard, state, canon, missing-context} and `<referent>` one of `continuity/book-N.md#co-NN`, `characters/<id>/knowledge/book-N.md#kn-NN`, `reveals.md#rv-NN`, or `canon/<file>` + a quote, or a scene-entry locator `<storyboard-file>#reader-state-in` / `<storyboard-file>#since-previous-scene` — the canonical surface form defined in `agents/review-context.md`. Checks 1–2 and Check 3's non-escalated common path (a bare `canon_active` comparison) do not carry the tag. Check 3's escalation path is the exception: when it resolves a marker-bearing `canon/**` entry, its violation line can carry a standalone `[premise: unreviewed]` tag and, for a `canon/generated/` referent, a freshly appended `[ref: <referent>]` tag — never a `[defect:]` tag, which Check 3 never carries (see Check 3, below). The anchor / `review-id` / `Decision:` scheme is identical for both (`agents/review-grammars.yaml` `compliance:` family).

The review-id follows the `compliance:` family segment grammar in `agents/review-grammars.yaml`: short_story form `compliance:<scene-id>:block-<NNN>-v<KK>`, book form `compliance:<book-id>:<chapter-id>:<scene-id>:block-<NNN>-v<KK>`. `<NNN>` is the storyboard block number and `<KK>` the violation's ordinal within that block's entry — both already in the report; the location segments are derivable from the artifact's resolved path. Emit `Decision:` and `Decision-note:` blank — they belong to the human, and a blank `Decision:` means the unit is pending review. The grammar-only fixture `examples/review/reviewer-actions.md` shows the review-unit shape; the complete chapter-wide prior-reader report in `examples/prior-reader-context/plot/drafts/attempt01/reviewer-actions.md` shows the scene sections, summary, and consulted scene-entry locators together.

Work block by block for the **local** checks (Must-Contain, Must-Not-Contain, Canon-as-stated): evaluate each block against its own prose range and do not collapse those findings across blocks. The **relational** check (Check 4) is different: it evaluates the **chapter as a whole** against maintained state + targeted retrieval, because the contradictions it catches are cross-scene by construction. **Scene-blind sharding is forbidden for relational checks** — splitting one chapter's continuity / recap / timeline / prior-reader evaluation into scene-blind shards makes cross-scene contradictions and redundant re-establishment structurally invisible (`NOTES.md:42-46`). A relational finding is still emitted as a per-block review unit anchored to the block whose prose carries it; "chapter-whole" describes the evaluation, not the anchoring.
...
**Escalation-path marker check.** Only on the escalation path — when `canon_active` is insufficient and the check opens a named `canon/**` file — read that file's **per-entry** marker for the specific entry the prose is checked against, never the file-level frontmatter `status:` (per the Confirmation lifecycle in `agents/update-rules.md`; detection acts on the per-entry inline tag only). Do not add this lookup on the common, non-escalated path (a bare `canon_active` comparison): per `agents/storyboard-schema.md:110-112`, `canon_active` holds an extracted rule, never a file path, so there is nothing there to resolve a marker from. If the escalation resolves an entry whose marker reads `unreviewed`, append `[premise: unreviewed]` to the violation line — standalone, since Check 3 carries no `[defect: <type>] [ref: <referent>]` tag today. If that entry lives in `canon/generated/` (which mints no addressable id), also append a new `[ref: canon/generated/<file>#<scene-beat-attempt> "<short quote of the entry's own text>"]` tag, built now while the entry is in hand — file path + the entry's own scene/beat/attempt provenance + a short quote of its text — because `compliance_fix` cannot read canon files to reconstruct this locator later and the `## Context consulted` section records only the file path, not the entry.

#### Check 4: Relational consistency

Checks 1–3 are **local** — each block against its own prose range. This check is **relational**: it reads the chapter's prose against facts established **elsewhere** — the maintained state (`continuity/`, `characters/<id>/knowledge/`), each scene's explicit prior-reader entry context, plus targeted retrieval of the named referents the prose invokes. The classification, referent-identification, targeted-retrieval, precedence, and defect-taxonomy strategy is single-sourced in `agents/review-context.md` and is **not** restated here — this step consults it. Evaluate the chapter as a whole (see the partitioning rule above); never in scene-blind shards.
...
- **Prior-reader re-establishment.** For each scene, read `Reader state in` and `Since previous scene` from its first block, then evaluate the scene's prose against those fields and the current `Beat`, `Must Preserve`, `Reader takeaway`, and `Craft signal`. Record `REDUNDANT (reader_state_in)` only when the prose presents listed prior-reader context as new, explains it again, or produces another establishing treatment **and** the current storyboard assigns that repetition no new dramatic purpose. A passing mention, action that naturally uses an established condition, or purposeful contrast, correction, reminder, or changed significance is not a violation. A material change in `Since previous scene` may require fresh orientation; do not flag it merely because related subject matter appeared earlier.

  Cite the scene-entry field as `[ref: <storyboard-file>#reader-state-in]` (or `#since-previous-scene` when the judgment turns on the delta), and name the earlier scene/block or established item in the finding text when available. Apply the normal defect taxonomy: **prose** when the entry context and current storyboard agree that repetition is unnecessary but the prose re-establishes it; **storyboard** when a current beat-level instruction itself demands purposeless repetition inconsistent with its own scene-entry context; **missing-context** when the scene-entry fields are absent, ambiguous, or insufficient to determine whether the treatment is repetition. Do not reconstruct a missing reader state by scanning prior prose or the corpus.

Emit each relational finding as a per-block review unit on the existing `compliance:` grammar (the `- INCONSISTENT …` or `- REDUNDANT …` shape above), its violation line carrying the ` [defect: <type>] [ref: <referent>]` tag. If the fact needed to judge is absent from every consulted source, label it `missing-context` and surface it as an open question rather than fabricating a verdict.
...
- Redundant prior-context re-establishments: N
...
`Redundant prior-context re-establishments` is a diagnostic subset of `Relational violations`, not an additional disjoint count. Include the line even when its value is zero.
...
Always list each scene-entry field locator consulted by the prior-reader sub-check, even when no maintained state or canon file exists. If no other relational source was consulted, the section contains only those storyboard-field locators; do not write `- none` after running the prior-reader sub-check. A deliberately incomplete grammar specimen such as `examples/review/reviewer-actions.md` is not a generated report run and must not fabricate locators for storyboards it does not have.
...
**Treating every repeated fact as redundant exposition.** Mentioning an established condition while a character acts on it is not the same as re-establishing it. Do not emit `REDUNDANT` when the current storyboard gives the repetition a new purpose, `Since previous scene` records a material change requiring orientation, or the prose merely uses context without explaining it again.

**Trusting a missing or contradictory scene-entry field.** If the review cannot determine the reader's prior state from the first block, emit a `missing-context` or `storyboard` finding as appropriate. Do not silently infer prior-reader state by rereading the corpus.
