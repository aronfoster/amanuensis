...
Process the chapter's scenes in declared order. In the first block of each scene, derive `Reader state in` from reader-visible outcomes of the preceding planned scene and derive `Since previous scene` from the boundary between that close and the current opening. At a chapter boundary, use only the targeted `<prior-scene-storyboards>` input plus the current chapter's declared boundary plans; never reconstruct the boundary from prior prose. Carry forward relevant material non-changes explicitly when omission would invite re-establishment or drift. These fields make each scene independently draftable; they are entry context, not recap requirements. The opening scene of the entire work uses the exact sentinels in `storyboard-schema.md`, while chapter boundaries retain real prior-reader context.
