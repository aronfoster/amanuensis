...
- **review_pending** — a review-sensitive artifact with no review evidence (an unannotated report, or any review-required output the human has not confirmed); surfaced as a notice, blocking only on the unannotated-report path. Resolved by the human adding review evidence (annotations, a valid bulk header, a selected variant), **not** by an override: an override waives a *state* problem, whereas `review_pending` is an absence of editorial intent, so there would be nothing for the consuming step to apply.
...
**Review is surfaced, not enforced.** The expectation is declared by `review_sensitive` (a frontmatter precondition key) and `review_gate` (a manifest field); evidence, where it exists, is the human's annotation on the four reports; a consuming step surfaces a non-blocking notice for a review-sensitive input and hard-blocks only via the pre-existing unannotated-report path. No positive "reviewed" field is added. For a family migrated to the structured review contract, the annotation that constitutes review evidence takes the form defined in `agents/review-grammars.yaml`: filled `- Decision:` fields on the artifact's review units.

**Override.** A human may authorize consuming a `stale` artifact — one whose editorial content is intact but stamped against an older draft — by recording an override; the override names the specific artifact and the draft mismatch and is written into the consuming step's apply log (the same place fix steps already record `Applied:` blocks). An override authorizes consuming despite a known *state* problem; it does not supply missing editorial intent, so it does **not** apply to a `review_pending` artifact — an unannotated report (or unselected variant set) has nothing to apply, and is resolved by adding review evidence, not by override. Overriding staleness waives the freshness block, not the requirement that each edit still land on a real anchor in `<latest-draft>`: an annotation whose anchor cannot be located is handled as it would be without the override, never guessed. No stale apply happens silently; absent a recorded override, the step blocks as today.
