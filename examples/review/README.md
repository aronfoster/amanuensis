...
| `anti-ai.md` | `anti_ai` (pending until M11) | book | `BULK eligibility` declarations; `BULK: FIX: rewrite` and `BULK: SKIP` on permitted categories; a per-entry `SKIP` override under a bulk header; a not-permitted category decided per entry | not-yet-adopted error, exit 1 |
...
The three `pending`-family fixtures also document, in their trailing
expectation comments, the ledger the validator must produce once their
milestone flips the adoption marker (verify by pointing the script at a
scratch copy of the grammar file with the marker flipped — never by editing
