...
# Generated Canon — Ashfall Road wards and gear
...
<!-- ILLUSTRATIVE EXAMPLE — NOT REAL PROJECT CANON OR STATE. See
     ...
     Entry A (Frost-ward duration) is shown CONFIRMED — the state after a /revise
     confirm-only pass; its text is unchanged, only its tag flipped. Entry B (Ward-sigil
     chalk) is shown still UNREVIEWED, untouched by that pass, proving confirmation is
     per-entry-only. See the README for Entry A's pre-confirmation text and the
     compliance finding that cited it. -->
